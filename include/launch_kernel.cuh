...
#define LAUNCH_KERNEL_MG_BLOCK_SIZE(kernel, tp, stream, arg, ...)	\
  ...
  case 96:								\
    ...
    break;								\
  case 100:								\
    kernel<100,__VA_ARGS__>						\
      <<< tp.grid, tp.block, tp.shared_bytes, stream >>>(arg);		\
    break;								\
  case 108:								\
    kernel<108,__VA_ARGS__>						\
      <<< tp.grid, tp.block, tp.shared_bytes, stream >>>(arg);		\
    break;								\
  case 128:								\
    kernel<128,__VA_ARGS__>						\
  ...
  case 512:								\
    ...
    break;								\
  default:								\
    errorQuda("%s block size %d not instantiated", #kernel, tp.block.x);\
  }
